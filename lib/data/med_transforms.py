    ...
    if args.dataset == 'btcv':
        if args.test:
            post_pred = transforms.Compose([transforms.EnsureType(),
                                            ...
                                            transforms.AsDiscrete(argmax=True, to_onehot=True, n_classes=args.num_classes)])
            post_label = transforms.Compose([transforms.EnsureType(),
                                            # Resize(scale_params=(args.space_x, args.space_y, args.space_z)),
                                            transforms.AsDiscrete(to_onehot=True, n_classes=args.num_classes)])
        else:
            post_pred = transforms.Compose([transforms.EnsureType(),
                                            transforms.AsDiscrete(argmax=True, to_onehot=True, n_classes=args.num_classes)])
            post_label = transforms.Compose([transforms.EnsureType(),
        ...
        post_pred = transforms.Compose([transforms.EnsureType(), transforms.Activations(sigmoid=True), transforms.AsDiscrete(threshold_values=True, logit_thresh=0.5)])

...
class DiceLoss(_Loss):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, pred_score: Optional[torch.Tensor] = None, token_predict: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        Args:
        ...
        if pred_score is not None:
            B = target.shape[0]
            with torch.no_grad():
                gt_edge = target.sum(dim=1).unsqueeze(1)
                pooled_max = F.max_pool3d(gt_edge, (8, 8, 8))
                pooled_avg = F.avg_pool3d(gt_edge, (8, 8, 8))
                gt_score = (pooled_max != pooled_avg).float()
                gt_token_pred = pooled_max.clone()
                gt_token_pred[pooled_max != pooled_avg] = 4.  # 14 4
            score_loss = F.binary_cross_entropy_with_logits(pred_score.reshape(B, 1, 16, 16, 16), gt_score)
...
class DiceCELoss(_Loss):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, pred_score: Optional[torch.Tensor] = None, token_predict: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        """
        if len(input.shape) != len(target.shape):   # [B, 14, 96, 96, 96] & [B, 1, 96, 96, 96]
            raise ValueError("the number of dimensions for input and target should be the same.")
        B = input.shape[0]
        n = 12

        dice_loss = self.dice(input, target)
        ce_loss_value = self.ce(input, target)
        ce_loss = torch.mean(ce_loss_value)
        if pred_score is not None:
            with torch.no_grad():
                target = target.sum(dim=1).unsqueeze(1) if n == 16 else target

...
def sliding_window_inference(
    ...
    **kwargs: Any,
    ...
    targets = F.pad(targets, pad=pad_size, mode=look_up_option(padding_mode, PytorchPadMode).value, value=cval)
    ...
    for slice_g in range(0, total_slices, sw_batch_size):
        ...
        window_gt = torch.cat([targets[win_slice] for win_slice in unravel_slice]).to(sw_device)
        seg_prob = predictor(window_data, window_gt, *args, **kwargs).to(device)  # batched patch segmentation

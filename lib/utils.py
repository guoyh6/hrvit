...
def get_vit_layer_id(var_name, num_max_layer, prefix=''):
    if var_name in (prefix + "cls_token", prefix + "mask_token", prefix + "pos_embed"):
        ...
        return block_id + 1
    elif var_name.startswith(prefix + "blks_def"):
        names = var_name.split('.')
        anchor_ind = names.index('blks_def') # 'blocks' is an anchor
        block_id = int(names[anchor_ind + 1])
        return block_id + 1
    elif var_name.startswith(prefix + "blks_sp"):
        names = var_name.split('.')
        anchor_ind = names.index('blks_sp') # 'blocks' is an anchor
        block_id = int(names[anchor_ind + 1])
        return block_id + 1
    elif var_name.startswith(prefix + "blks_enc"):
        names = var_name.split('.')
        anchor_ind = names.index('blks_enc') # 'blocks' is an anchor
        block_id = int(names[anchor_ind + 1])
        return block_id + 1
    else:
        return num_max_layer - 1

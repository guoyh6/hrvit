...
from networks.dice import DiceCELoss, DiceLoss
...
def save_model_py(output_dir):
    pyfiles = ['lib/models/adaseg3d.py', 'lib/networks/adavit.py',
               'configs/adaseg3d_btcv.yaml', 'lib/networks/dice.py']
    for f in pyfiles:
...
class SegTrainer(BaseTrainer):
    ...
    def run(self):
        ...
        niters = args.start_epoch * self.iters_per_epoch  # *9

        save_model_py(args.output_dir)
        best_metric = 0
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if (epoch + 1) % args.eval_freq == 0:
                metric = self.evaluate(epoch=epoch, niters=niters)
                if metric > best_metric:
                    with open(args.output_dir + '/best_metric.txt', 'a') as file:
                        file.write(f"This is epoch: {epoch}\n")
                        file.write(f"=> New best metric: {metric} | Old best metric: {best_metric}!\n")
                    print(f"=> New best metric: {metric} | Old best metric: {best_metric}!")
                    best_metric = metric
    ...
    def epoch_train(self, epoch, niters):
        ...
        for i, batch_data in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(True):
                if args.score_loss:
                    loss = self.train_score_batch(model, image, target, loss_fn)
                else:
            ...
            if i % args.print_freq == 0:
                ...
                print(f"Epoch: {epoch:03d}/{args.epochs} | "
                      ...
                      f"Init Lr: {self.lr:.07f} | "
                      f"Lr: {last_layer_lr:.07f} | "
        ...
        for i, batch_data in enumerate(val_loader):
            ...
            for metric_name, metric_func in self.metric_funcs.items():
                metric = metric_func(y_pred=output_convert, y=target_convert,
                                     include_background=False if args.dataset == 'btcv' else True)
                metric = metric.cpu().numpy()
                compute_avg_metric(metric, meters, metric_name, batch_size, args)
                for k in range(metric.shape[-1]):
                    cls_metric = np.nanmean(metric, axis=0)[k]
                    if np.isnan(cls_metric) or np.isinf(cls_metric):
                        continue
                    meters[f'{idx2label[k]}.{metric_name}'].update(value=cls_metric, n=batch_size)
    ...
    def mask2image(self, images, targets, outputs, id, channel_ind=0, directory='seg_vis_policy'):
        ...
        for image_t, target_t, output_t in zip(image_list, target_convert, output_convert):
            ...
            for i, ratio in enumerate([0.3, 0.4, 0.5, 0.6, 0.7]):
                ...
                # pdb.set_trace()
                print("start saving")
                np.savez(os.path.join(directory, f'image_bid{id}_depid{i}'), image=image, target_mask=target_mask,
                         output_mask=output_mask, target_alphas=target_alphas, output_alphas=output_alphas)
    ...
    def vis_policy(self, niters=0, modality_idx=0, image_alpha=0.5):
        ...
        for b_idx, batch_data in enumerate(loader):
            ...
            output, policy_list = model(image,
                                        vis_policy=True)  # policy list of [num_layers+1] 0-or-1 tensors with shape of [B, L+1]
    ...
    def speedometerv2(self):
        ...
        for key in time_meters.keys():
            ...
            print(f"=> averaged inference time for {key} is {avg_time}")
        print(f"MACS is {macs} G")
        print(f"{2 / np.mean(time_meters['total'])} total")
        print(f"{2 / np.mean(time_meters['enc'])} enc")
        # print(f"==> avg inference time over all trials is {np.mean(trials)}")
